...
block manifest, and `manifests/t1-signoff-report.json` is the rendered
report frozen at the repo's pinned `klt` (see `manifests/README.md`). This
script re-renders the report from the manifest right now and refuses to
let the frozen one rot:

  exit 0   fresh render succeeded (exit 0 or 3 -- both are clean runs,
           see below) AND equals the committed report semantically.
  exit 1   anything else: the render errored (exit 1), OR the fresh
           output diverges from the committed report.

Exit 3 from `klt signoff --manifest` means "report rendered fine, the
block is not yet at tier T1" -- that is this repo's *expected* state and
NOT a failure of this check. A change in any cited artifact's input
(hash-pin mismatch) flips the affected item to `unmet`/`stale_evidence`
in the fresh render, which then no longer equals the committed report --
exactly the failure this check exists to catch, per issue #78's "CI
re-runs it, so a manifest citing an artifact that has since changed fails
rather than rotting" acceptance criterion. A stale committed report (old
`klt` pin, moved checklist, regenerated evidence) fails the same way:
regenerating the report and updating the manifest move in one change.

Zero dependencies beyond the Python 3 standard library and a working
`klt` on `$PATH` (or `--klt <path>`).

Usage:
    python3 scripts/check_signoff_report.py
    python3 scripts/check_signoff_report.py --klt .venv/bin/klt
...
DEFAULT_MANIFEST = REPO_ROOT / "manifests" / "gf180-usb2-phy.json"
...
#: and 0 for every T1 item met. 1 is an error.
_ACCEPTED_EXITS = (0, 3)
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="block manifest to grade (default: manifests/gf180-usb2-phy.json)",
    )
    ap.add_argument(
        "--report",
    ...
    ap.add_argument(
        "--klt",
        default="klt",
        help="klt executable (default: 'klt' from $PATH)",
    )
    args = ap.parse_args()
