...
`layout/analog/plans/*.json` are the resulting committed plans — as of issue
#72, all five blocks now have one. The three receiver plans use one
`device_groups[]` entry per netlist device (`mos_array` with `rows`/`cols` 1
and `dummy` 0 for each MOS, `res_array` with `num` 1 and `dummy` 0 for each
poly resistor), every group in a single `rows[]` row. `dplus_pullup.json`
(issue #62) is the first plan that groups rather than enumerates: 78 devices
in 24 groups — the six differently-sized `ppolyf_u` ladder segments as six
distinct `res_array` groups (one shared group would force a single
`length_um` onto all six), each switch bank's ten flattened fingers as one
10-wide `mos_array`, and the six control inverters as per-device groups
clustered in n/p pairs. It is also the first plan to declare `routing`
explicitly rather than take klt's default — see "Route width" below.
`differential_driver.json` (issue #72) is the last of the five to be
authored — one `device_groups[]` entry per netlist device like the three
receiver plans, with one addition: its two `RM1`-classed series-termination
resistors set `params.metal_level: 1` explicitly, the field klayout-tools#1731's
fix added drawn-geometry support for (see "Friction filed" below); without
it, `res_array` would silently draw a poly resistor instead of the metal-1
device the netlist declares. `scripts/gen_analog_layout.py` executes them
and runs `klt drc` on each result. Nothing here is hand-drawn: `klt draw`
remains the wrong tool for a layout claim (its own docs: "no PDK awareness
and no rule checking").

### What actually happened

Nine runs exist. The first two ran against two `klt` pins over
...
the sixth run did. The ninth (issue #72) changes `scripts/setup-env.sh`'s
`KLT_REV` again, moved to `61e743088f47` to consume klayout-tools#1731
(`res_array` drawn-geometry support for gf180mcu metal resistors, plus the
declared-class-vs-resolved-generator mismatch warning) — with the four
already-committed plan documents byte-identical (isolating the pin bump's
drawn-geometry effect on them, as before) — **and** adds the fifth plan,
`differential_driver.json`, in the same run.

| Block | 2026-08-18, `klt` 0.2.0 @ `b3e284f` | 2026-08-26, `klt` 0.3.0 @ `07b1f04` | 2026-09-05, flattened `dplus_pullup` | 2026-09-05, + committed `dplus_pullup` plan | 2026-09-05, + legal `routing.width_um` (issue #65) | 2026-09-09, `klt` 0.4.0 @ `8bd3f0b0`, plans byte-identical (issue #68) | 2026-09-09, + `cross_block_layer_role` on `dplus_pullup` (issue #68) | 2026-09-12, `klt` 0.4.0 @ `bf90e1d5e3a6`, plans byte-identical (issue #70) | 2026-09-13, `klt` 0.4.0 @ `61e743088f47`, four plans byte-identical + new `differential_driver` plan (issue #72) | 2026-09-14, `klt` pin unchanged, `params.flavor: "1k"` fix on three plans (issue #73) | 2026-09-21, `klt` 0.5.0 @ `e8ca621a`, plans byte-identical (issue #78 re-pin, move 6) |
|---|---|---|---|---|---|---|---|---|---|---|
| `differential_receiver` | 11 groups placed, **DRC-clean**, **0/8 nets routed** | 11 groups placed, **0/8 nets routed (unchanged)**, DRC **19 violations** (`metal1.width.1`) | unchanged — byte-identical GDS | unchanged — byte-identical GDS | **DRC-clean (0 violations)**, **0/8 nets routed (unchanged)** — `routing.width_um` corrected from klt's illegal 0.17 µm default to the deck's own 0.23 µm minimum | **1/8 nets routed** (`VDD`), DRC-clean — the pin bump alone; geometry re-floored at deck minimums, plan untouched | unchanged — plan untouched by this run | unchanged — `cmp`-identical GDS; plan untouched | unchanged drawn geometry — geometrically identical GDS (per-layer Boolean XOR: zero area difference on all 10 layers; **not** byte-identical — same elements, different emission order), plan untouched; **new**: `klt`'s new mismatch check reports 1 `warnings[]` entry (`r_bias` declares `PPOLYF_U_1K`, draws default-flavor `ppolyf_u`) — a pre-existing mismatch, not a regression; not fixed here (see "Friction filed") | **fixed**: `r_bias` gains `params.flavor: "1k"` — now draws real `ppolyf_u_1k` geometry (layer `31/0` → `62/0` swap, confirmed by direct GDS read), **0 `warnings[]`** (down from 1); **1/8 nets routed (unchanged)**, **DRC-clean (unchanged)**, bbox unchanged (per-layer XOR against the prior run is zero everywhere except the two swapped layers) | 1/8 nets routed (unchanged), DRC-clean, 0 `warnings[]`, bbox unchanged — `cmp`-identical GDS to the tenth run; the pin bump does not move this block at all |
| `se_receiver_dm` | 13 groups placed, **DRC-clean**, **0/9 nets routed** | 13 groups placed, **0/9 nets routed (unchanged)**, DRC **22 violations** | unchanged — byte-identical GDS | unchanged — byte-identical GDS | **DRC-clean (0 violations)**, **0/9 nets routed (unchanged)** — same fix | **1/9 nets routed** (`VDD`), DRC-clean — same mechanism | unchanged — plan untouched by this run | unchanged — `cmp`-identical GDS; plan untouched | unchanged drawn geometry — geometrically identical GDS (per-layer Boolean XOR: zero area difference on all 10 layers; **not** byte-identical — same elements, different emission order), plan untouched; **new**: 3 `warnings[]` entries (`r_bias`/`r_1`/`r_2`, same mismatch shape as above) | **fixed**: all three groups (`r_1`, `r_2`, `r_bias`) gain `params.flavor: "1k"` — same layer-swap confirmation, **0 `warnings[]`** (down from 3); **1/9 nets routed (unchanged)**, **DRC-clean (unchanged)**, bbox unchanged | 1/9 nets routed (unchanged), DRC-clean, 0 `warnings[]`, bbox unchanged — `cmp`-identical GDS to the tenth run |
| `se_receiver_dp` | 13 groups placed, **DRC-clean**, **0/9 nets routed** | 13 groups placed, **0/9 nets routed (unchanged)**, DRC **22 violations** | unchanged — byte-identical GDS | unchanged — byte-identical GDS | **DRC-clean (0 violations)**, **0/9 nets routed (unchanged)** — same fix | **1/9 nets routed** (`VDD`), DRC-clean — same mechanism | unchanged — plan untouched by this run | unchanged — `cmp`-identical GDS; plan untouched | unchanged drawn geometry — geometrically identical GDS (per-layer Boolean XOR: zero area difference on all 10 layers; **not** byte-identical — same elements, different emission order), plan untouched; **new**: 3 `warnings[]` entries (`r_bias`/`r_1`/`r_2`, same mismatch shape as above) | **fixed**: all three groups (`r_1`, `r_2`, `r_bias`) gain `params.flavor: "1k"` — same layer-swap confirmation, **0 `warnings[]`** (down from 3); **1/9 nets routed (unchanged)**, **DRC-clean (unchanged)**, bbox unchanged | 1/9 nets routed (unchanged), DRC-clean, 0 `warnings[]`, bbox unchanged — `cmp`-identical GDS to the tenth run |
| `differential_driver` | **cannot be ingested** — series-termination resistors are `rm1` (metal-1) devices, unknown to klt's curated `gf180mcu` deck | **cannot be ingested — identical error text, verbatim** | **still cannot be ingested** — unrelated blocker, unchanged | **still cannot be ingested** — unchanged | **still cannot be ingested** — unchanged, out of this fix's scope | **still cannot be ingested — identical error text, verbatim**; reconfirmed at the new pin and filed upstream as klayout-tools#1621 | unchanged | **now ingests** (klayout-tools#1640/PR #1662 fixed) — 12-device digest resolves cleanly for the first time. Still **no committed plan**: a throwaway probe places 12 groups / **0/10 nets routed** / DRC-clean, but `klt gen`'s `res_array` has no gf180mcu metal-resistor generator support and *silently draws the wrong device* (a poly resistor) for the two `rm1` groups when left at default `flavor` — filed generically as klayout-tools#1731 | **plan committed** (`differential_driver.json`, `params.metal_level: 1` on both `RM1` groups) — 12 groups placed, **0/10 nets routed**, DRC-clean, **0 `warnings[]`**: both series-termination resistors confirmed (by direct GDS layer inspection) to draw real `Metal1`/`Via1`/`Metal2` geometry, not the poly substitution the eighth run caught | unaffected — plan untouched, `cmp`-identical GDS (issue #73 is scoped to the three receiver plans' `PPOLYF_U_1K` resistors only) | 0/10 nets routed (unchanged), DRC-clean, 0 `warnings[]`, bbox unchanged — `cmp`-identical GDS to the tenth run |
| `dplus_pullup` | **cannot be ingested** — pull-up switches carry `nf=10`, which klt's subckt-call → plain-element conversion refuses to represent | **cannot be ingested — identical error text, verbatim** | **ingests, and places** — 78 devices / 21 nets; blocker cleared by the flatten. Still **no committed plan**, so still no layout | **plan committed and executed** — 24 groups placed, **6/21 nets routed**, **DRC-clean (0 violations)**. Still not a layout: 15 nets unrouted | unchanged — byte-identical GDS (its plan already carried the legal `routing.width_um: 0.23`) | **6/21 nets routed (unchanged)**, DRC-clean; 114/1301 legs routed | **6/21 nets routed (still unchanged)**, DRC-clean **with `via1`/`metal2` rules now actually checked**; **168/769 legs routed**, the 540 same-layer-short leg rejections **gone**, 2 nets `unrouted` → `partial` | unchanged — `cmp`-identical GDS; plan untouched | unchanged drawn geometry — geometrically identical GDS (per-layer Boolean XOR: zero area difference on all 13 layers; **not** byte-identical — same elements, different emission order), plan untouched; 0 `warnings[]` (its `res_array` groups declare plain `PPOLYF_U`, matching the default flavor they draw — no mismatch) | unaffected — plan untouched, `cmp`-identical GDS (its `res_array` groups already declared plain `PPOLYF_U`, no mismatch to fix) | **6/21 nets routed (unchanged), DRC-clean, 0 `warnings[]`, bbox unchanged** — but the *drawn routing* genuinely moved with the pin: per-layer XOR vs the tenth run is zero on every layer except `34/0` (Metal1), which loses 63,504,000 dbu² (610,832,320 → 547,328,320) — the four already-routed ladder nets `NODE2`–`NODE5` are re-legged far shorter (`route_length_um` 227.1→75.5, 234.0→76.2, 247.7→77.5, 454.8→65.2) by the upstream routing-semantics commits that landed between the pins (klayout-tools#1931/#1936/#1945/#2098/#2144: legs reaching a pin from the side it doesn't face are now rejected, own-block approaches held to the deck's spacing rule, routed metal floored against per-layer min-area). No recorded status moved: the 15 unrouted nets stay unrouted and stay the (d) critical path |

**New finding, ninth run: three of the four already-committed plans have their own declared-class/drawn-device mismatch, just found by the same check that fixed `differential_driver`.** `differential_receiver.json`'s `r_bias` group and `se_receiver_dm.json`/`se_receiver_dp.json`'s `r_bias`/`r_1`/`r_2` groups all declare `device_class: "PPOLYF_U_1K"` (matching the netlist, which genuinely instantiates those bias/reference resistors as the `ppolyf_u_1k` model — a specific 1 kΩ/sq flavor) but never set `params.flavor: "1k"`, so `res_array`'s default `"generic"` flavor has silently drawn the wrong poly-resistor body for all seven groups since before this pin moved. The drawn geometry for these three blocks is unchanged by this pin bump (per-layer Boolean XOR against the superseded record's artifacts: zero area difference on every layer; the GDS files are geometrically identical, not byte-identical) — this is not a regression, it is a standing, previously-undetectable defect the new mismatch-warning check (klayout-tools#1731's second half) surfaces for the first time. **Not fixed in this run**: correcting it is real per-block layout work (each group needs a `params.flavor: "1k"` override, then its own DRC/LVS re-verification), out of scope for issue #72, and tracked by [gf180-usb2-phy#73](https://github.com/2AMLogic/gf180-usb2-phy/issues/73) — see record `verification/records/analog-layout/records/20260913-192744-b653c71.md`.

**Fixed (2026-09-14, issue #73, tenth run).** All seven affected groups now carry `params.flavor: "1k"`. Re-executing the three plans confirms the mismatch check reports **0 `warnings[]` entries** on all three (down from 1/3/3), each affected group's drawn body swaps from `ppolyf_u`'s layer `31/0` to `ppolyf_u_1k`'s layer `62/0` marker (confirmed by direct GDS layer inspection against each group's own bounding box), and nothing else moves: routed-net counts (1/8, 1/9, 1/9), DRC-clean status, device-group counts, and block bboxes are unchanged from the superseded record, and a per-layer Boolean XOR against its frozen artifacts is exactly zero on every layer except the one `31/0`→`62/0` swap per block (equal area on each side, i.e. a pure body-flavor substitution, not a resize). See record `verification/records/analog-layout/records/20260914-023747-74372a3.md` for the full reproduction. Routing any of the pre-existing unrouted nets on these three blocks remains out of scope for this fix (unchanged by it, and not something a `flavor` correction touches).

**Eleventh run (2026-09-20/21, `klt` re-pin move 6, issue #78).** Nothing
design-side changed; this run exists because the `klt` pin moved to
`e8ca621a` (the revision that can grade T1 item 11 — see
`docs/environment-setup.md`), and the re-measure discipline applies. All
five plans re-executed with input hashes byte-identical to the tenth run:
four blocks reproduce `cmp`-identical GDS, and `dplus_pullup`'s drawn
Metal-1 routing re-shaped under the upstream routing-semantics commits
between the pins (`route_length_um` collapses on `NODE2`–`NODE5`,
63,504,000 dbu² less layer `34/0`) with **no change to any recorded
status** — 6/21 routed stays 6/21, DRC stays clean, `warnings[]` stays 0,
bboxes stay put. See record
`verification/records/analog-layout/records/20260921-130120-223bae5.md`
for the full reproduction, and `manifests/README.md` for the graded
T1 state this era exists to track.
...
- [klayout-tools#1731](https://github.com/2AMLogic/klayout-tools/issues/1731)
  (filed by issue #70; **closed via PR #1774, merge `1641fa5f`, consumed by
  issue #72's pin move to `61e743088f47`**) — landing #1621's fix had moved
  the blocker one stage downstream rather than clearing it: `klt gen`'s
  `res_array` generator had no drawn-geometry path for a gf180mcu metal
  resistor at all (its `metal_level` mechanism was populated for a
  different PDK family only), and — the sharper half of this gap — a
  `device_groups[]` entry whose declared `device_class` named a
  metal-resistor class but whose `generator`/`flavor` resolved to the
  default poly-body path did not fail; it **silently drew the wrong
  device** (a poly resistor) with no error and no `warnings[]` entry,
  because nothing cross-checked a group's declared device class against
  what its generator actually drew. **Fixed and consumed**: `res_array`'s
  `metal_level` mechanism now covers gf180mcu's `rm1`/`rm2`/`rm3`, and a new
  `_device_class_mismatch_warnings` check appends a `warnings[]` entry
  instead of silently substituting whenever a declared class disagrees with
  what the resolved generator/flavor actually draws. Confirmed directly by
  reading `klt gen`'s own source (`_PDK_METAL_RES_LEVELS`/
  `_PDK_METAL_RES_DEVICE_CLASS` in `gen.py`) and by executing
  `differential_driver`'s committed plan, not assumed — see record
  `20260913-192744-b653c71`. This is why `differential_driver` now has a
  committed plan under `layout/analog/plans/`: `res_array` with an explicit
  `params.metal_level: 1` draws its two series-termination resistors as the
  real `rm1` devices the netlist and LVS both expect, confirmed by direct
  GDS layer inspection. **The new mismatch-warning half of the fix also
  found a second, unrelated, pre-existing bug it was not filed to find**:
  three of the four already-committed plans
  (`differential_receiver.json`'s `r_bias`; `se_receiver_dm.json`/
  `se_receiver_dp.json`'s `r_bias`/`r_1`/`r_2`) declare `device_class:
  "PPOLYF_U_1K"` but never set `params.flavor: "1k"`, so `res_array` has
  been silently drawing the default `"generic"`-flavor `ppolyf_u` body
  instead — the identical mismatch shape `differential_driver`'s `RM1`
  groups had, just on `flavor` instead of `metal_level`, and on
  already-committed plans rather than one that was blocked from being
  committed at all. This is **not** a further `klt` gap (the tool now
  correctly reports it); it is this repo's own plan documents disagreeing
  with the netlist they were written against. Not fixed here — real
  per-block layout work, tracked by
  [gf180-usb2-phy#73](https://github.com/2AMLogic/gf180-usb2-phy/issues/73)
  (see record `20260913-192744-b653c71`'s "Links"). **Fixed 2026-09-14**:
  see the "New finding, ninth run" / "Fixed" update above and item (f)
  below for the corrected state (record `20260914-023747-74372a3`).
...
**Several of the items this list used to carry have now closed, and closing
them did not deliver a layout** — which is itself the most useful thing this
section records. As of 2026-09-13 (issue #72), what remains is:
...
(c′) **~~`klt gen` drawing a metal-resistor device for the family whose
extraction side just gained one, and validating that a plan's declared
`device_class` matches what its generator actually draws~~ — CLOSED.**
klayout-tools#1731 closed via PR #1774 (merge `1641fa5f`), consumed by
issue #72's pin move to `61e743088f47`: `res_array` now draws real
`Metal1`/`Via1`/`Metal2` geometry for a `metal_level`-scoped gf180mcu
resistor, and `differential_driver.json` (issue #72) is committed with an
explicit `params.metal_level: 1` on both `RM1` groups, confirmed to draw
the correct device by direct GDS layer inspection, not assumed. All five
analog blocks now have a committed plan.

(f) **~~The mismatch-warning half of #1731's fix found a second,
unrelated, pre-existing bug — `flavor` overrides missing on three
already-committed plans' `PPOLYF_U_1K`-declared resistors~~ — CLOSED.**
`differential_receiver.json`'s `r_bias` group and
`se_receiver_dm.json`/`se_receiver_dp.json`'s `r_bias`/`r_1`/`r_2` groups
declared `device_class: "PPOLYF_U_1K"` (matching the netlist) but never set
`params.flavor: "1k"`, so they had been silently drawing the default
`"generic"`-flavor `ppolyf_u` body instead — present since before issue
#72's pin move, only made visible by it. Not a `klt` gap (the tool
correctly reported it); this repo's own plan documents needed a
one-field-per-group fix plus re-verification. **Fixed by
[gf180-usb2-phy#73](https://github.com/2AMLogic/gf180-usb2-phy/issues/73)**:
all seven groups now carry `params.flavor: "1k"`, confirmed by direct GDS
layer inspection to draw `ppolyf_u_1k` (not `ppolyf_u`), with the
`_device_class_mismatch_warnings` check now reporting zero entries on all
three plans and DRC/routed-net counts/bboxes unchanged (see record
`20260914-023747-74372a3`, superseding `20260913-192744-b653c71`'s
still-open finding).
...
As of issue #72, `BLOCKED_BLOCKS` in `scripts/gen_analog_layout.py` is empty
— every one of the five analog blocks now executes from a committed plan
(`differential_driver.json` was the last to be authored, once
klayout-tools#1731 gave `res_array` drawn-geometry support for gf180mcu
metal resistors). The script's `_probe_blocked()` re-ingestion path (and
the `BLOCKED_BLOCKS` dict itself) is kept in the driver rather than deleted,
so a future block that regresses to "cannot be ingested"/"no plan possible"
has somewhere to land without re-deriving the harness shape.
