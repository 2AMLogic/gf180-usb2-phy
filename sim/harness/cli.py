...
def run(args: argparse.Namespace) -> int:
    ...
    if not args.quiet:
        ...
        print(f"temps (C) : {', '.join(report.fmt(t) for t in temperatures)}")
        print(f"supply (V): {', '.join(report.fmt(v) for v in supplies)} "
              f"(nominal {report.fmt(nominal)} +/-{tolerance * 100:g}%)")
    ...
    def progress(result):
        ...
        if result.status == "ok":
            detail = "  ".join(
                f"{name}={report.fmt(result.measurements[name])}" for name in tb.measure
    ...
    for name, stats in record["summary"].items():
        if not stats.get("n"):
            ...
            continue
        print(
            f"  {name:<16}{report.fmt(stats['min']):>16}{report.fmt(stats['max']):>16}"
            f"{report.fmt(stats['mean']):>16}{report.fmt(stats['spread_pct']):>12}"
        )

    for failure in record["checks"]["failures"]:
        print(
            f"  CHECK FAIL {failure['measurement']} {failure['kind']}={report.fmt(failure['limit'])} "
            f"got {report.fmt(failure['value'])} at {failure['at']}"
        )
